...
describe('rateLimit()', () => {
  it('permite primeira requisição', () => {
    ...
    const result = rateLimit(uid, { max: 3, windowMs: 60_000 })
  ...
  it('decrementa remaining a cada requisição', () => {
    ...
    const r1 = rateLimit(uid, { max: 5, windowMs: 60_000 })
    const r2 = rateLimit(uid, { max: 5, windowMs: 60_000 })
    const r3 = rateLimit(uid, { max: 5, windowMs: 60_000 })
  ...
  it('bloqueia após atingir limite', () => {
    ...
    rateLimit(uid, { max: 2, windowMs: 60_000 })
    rateLimit(uid, { max: 2, windowMs: 60_000 })
    const result = rateLimit(uid, { max: 2, windowMs: 60_000 })
    expect(result.success).toBe(false)
    expect(result.remaining).toBe(0)
  })

  it('continua bloqueando após limite atingido', () => {
    const uid = `keep-blocked-${Date.now()}-${Math.random()}`
    rateLimit(uid, { max: 1, windowMs: 60_000 })
    const r2 = rateLimit(uid, { max: 1, windowMs: 60_000 })
    const r3 = rateLimit(uid, { max: 1, windowMs: 60_000 })
    expect(r2.success).toBe(false)
    expect(r3.success).toBe(false)
  })

  it('usa defaults quando config não fornecida (max=10)', () => {
    const uid = `default-${Date.now()}-${Math.random()}`
    const result = rateLimit(uid)
    expect(result.success).toBe(true)
    expect(result.remaining).toBe(9)
  })

  it('isola contadores por identifier diferente', () => {
    const uid1 = `isolate-a-${Date.now()}-${Math.random()}`
    const uid2 = `isolate-b-${Date.now()}-${Math.random()}`
    rateLimit(uid1, { max: 1, windowMs: 60_000 })
    const blocked = rateLimit(uid1, { max: 1, windowMs: 60_000 })
    const allowed = rateLimit(uid2, { max: 1, windowMs: 60_000 })
  ...
  it('reseta janela após windowMs expirar', async () => {
    ...
    rateLimit(uid, { max: 1, windowMs: 50 })
    const blocked = rateLimit(uid, { max: 1, windowMs: 50 })
    expect(blocked.success).toBe(false)

    await new Promise(r => setTimeout(r, 60))

    const after = rateLimit(uid, { max: 1, windowMs: 50 })
