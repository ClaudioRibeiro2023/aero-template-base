...
import { Button, PageHeader, Alert, Card } from '@template/design-system'
    ...
    <div className="min-h-screen bg-surface-base">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
        ...
        <Alert
          ...
          title={`Status Geral: ${STATUS_CONFIG[overallStatus].label}`}
          description={`${MOCK_HEALTH.filter(h => h.status === 'healthy').length} de ${MOCK_HEALTH.length} serviços operacionais`}
          className="mb-6"
        />
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {MOCK_HEALTH.map(check => {
            ...
            return (
              <Card key={check.name} variant="outlined" padding="none" className={config.bg}>
                <div className="p-4">
                  <div className="flex items-start justify-between mb-3">
                    <div className="flex items-center gap-2">
                      <ServiceIcon size={18} className="text-text-muted" />
                  ...
                  {check.message && (
                    <p className="mt-2 text-sm text-text-secondary">{check.message}</p>
                  )}
                      ...
                      <div className="grid grid-cols-2 gap-2 text-xs">
                        {Object.entries(check.details).map(([key, value]) => (
                          <div key={key}>
                            ...
                            <span className="font-medium text-text-secondary">{String(value)}</span>
                          </div>
                        ))}
                      </div>
                    </div>
        ...
        {/* Endpoints */}
        <div className="mt-6 bg-surface-elevated rounded-lg border border-border-default p-6">
          <h3 className="text-lg font-medium text-text-primary mb-4">Endpoints de Health Check</h3>
          <div className="space-y-2 font-mono text-sm">
            <div className="flex items-center gap-3 p-2 bg-surface-muted rounded">
              <span className="text-color-success">GET</span>
              <span className="text-text-secondary">/health/live</span>
              <span className="text-text-muted">→ Liveness probe (K8s)</span>
            </div>
            <div className="flex items-center gap-3 p-2 bg-surface-muted rounded">
              <span className="text-color-success">GET</span>
              <span className="text-text-secondary">/health/ready</span>
              <span className="text-text-muted">→ Readiness probe (dependências)</span>
            </div>
            <div className="flex items-center gap-3 p-2 bg-surface-muted rounded">
              <span className="text-color-success">GET</span>
              <span className="text-text-secondary">/metrics</span>
              <span className="text-text-muted">→ Prometheus metrics</span>
            </div>
          </div>
        </div>
      </div>
